...
_SHARED_RULES = (
    ...
    "- drill 도구의 name/title 파라미터에는 **list_* 가 반환한 정식 이름 그대로** 사용.\n"
    "  사용자 발화의 약칭(예: '앤', '애번리')을 그대로 넣지 말고 peek 결과의 풀네임\n"
    "  (예: '앤 셜리', '애번리 마을')으로 변환해 호출.\n"
    "- 회차 시간순(이전화/이후화) 판단은 오직 **작가가 title 안에 적은 회차 표기** 만 근거.\n"
    "  sort_order, created_at, 도구 결과의 배열 순서 모두 시간순으로 해석 금지 — 드래그·\n"
    "  작성 시점·정렬 옵션에 따라 달라지는 표면 메타에 불과.\n"
    "- 직전화 = 검사 대상 title 의 회차 번호 N - 1 매칭. title 에 회차 번호 표기 없는 회차는\n"
    "  시간순 판단 불가 → 일반 인용은 가능, 이전화/이후화 분류에 쓰지 마라.\n"
...
SCENARIOS: dict[str, dict] = {
    "auto": {
        ...
        "system_prompt": (
            ...
            "**진짜 모순이 없으면 issues=[] 가 정답이다. 무리하게 1건을 만들지 마라.\n"
            "잘못된 등록 1건이 누락 5건보다 큰 손해다.** 인물의 내면 추측·잘못된 가정이 이후\n"
            "발화·사실로 정정되는 흐름, 자유간접화법, 의문문, '~일까/~었을지도' 같은 추측 어미,\n"
            "시점 인물의 제한된 정보·오해는 작가가 설계한 narrative 구조 — 모순 아님.\n\n"
            "탐색 순서:\n"
    ...
    "consistency_check": {
        ...
        "system_prompt": (
            ...
            "     2차로 벡터 보강.\n"
            "   - 시간선·복선 의심: 시계열·복선 추적 도구. 복선은 단서 강도가 약한 것이 정상 —\n"
            "     작가 등록 복선(foreshadow 노트) 과 본문 일관성으로 판단, 단서가 얇다는 이유로\n"
            "     누락하지 마라. 검사 대상 회차의 title 회차 번호보다 큰 회차의 회수 정보는\n"
            "     검수 자료에서 배제.\n"
            "   - 흐름·문맥 의심: 직전화·인접 회차 본문 (단 title 회차 번호 < N 인 것만).\n"
            ...
            "   채택 게이트 — 본문 라인 인용 + 도구 검증 결과 둘 다 명백할 때만. 한쪽이라도 모호하면 skip.\n"
            "   여러 건은 한 iter 에 병렬 propose 권장.\n"
            "   severity 는 한 단계 보수적으로 (헷갈리면 critical→warning, warning→info). issue_type 은 카테고리에 따라 자율 판단 (도구 enum 참조).\n"
