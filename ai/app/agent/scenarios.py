...
SCENARIOS: dict[str, dict] = {
    "auto": {
        ...
        "system_prompt": (
            ...
            "## ★ 회차 본문 작성 규칙 (anthropic streaming 우회 — 필수 준수)\n"
            "회차 본문 (propose_episode_draft) 을 작성할 때:\n"
            "1. 먼저 본문 전체를 **자연어 Markdown 으로 직접 출력** 하세요 (assistant 응답 텍스트로).\n"
            "2. 본문 출력이 끝나면 **propose_episode_draft({title}) 만 호출** — content 인자는 비웁니다.\n"
            "3. 백엔드가 직전 자연어 본문을 자동으로 content 로 합성해 큐에 적재합니다.\n"
            "이유: anthropic API 는 tool input JSON 을 buffering 후 한 번에 전송 (token streaming 불가).\n"
            "본문을 도구 인자에 채우면 사용자가 본문이 한 번에 나타나는 걸 보게 됩니다 (UX 나쁨).\n"
            "본문을 자연어로 출력하면 token 별 라이브 streaming 가능.\n"
            "이 규칙은 propose_episode_draft 에만 적용 — 다른 propose_* (인물/세계관/플롯) 는\n"
            "지금까지처럼 input 인자에 직접 채워 호출.\n\n"
            + _SHARED_RULES
            + "\n## 응답\n"
