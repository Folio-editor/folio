...
async def request_episode_summary_backfill(
    ...
    *,
    ...
    엄격한 트리거 조건 (운영 토큰 폭주 방지):
      - **episode.status = '완성'** 만
      - episode_summary 행 없음 (신규) OR episode.updated_at > es.last_generated_at (stale)
      - limit (기본 50, 상한 200) — 시간 순으로 N개 큐 적재
    """
    ...
    sql = (
        "SELECT ep.id, ep.title, ep.status, "
        ...
        "  AND ep.status = '완성' "
        "  AND (es.episode_id IS NULL OR ep.updated_at > es.last_generated_at) "
