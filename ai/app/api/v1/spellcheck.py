...
**확신 9할 미만 = 무조건 skip.** 헷갈리는 표기는 전부 작가 의도로 간주하라.
잘못된 수정을 제안하는 것이 비어 있는 결과보다 훨씬 큰 손해다.

각 issue 출력 직전 자가 점검 (한 단계라도 No 면 무조건 skip):
...
5. **활용형 자가 검증** — 변경 대상이 동사·형용사의 활용이라면 같은 어간의 다른 활용 (~서 / ~고 / ~지만 / ~며 / ~기 등) 에 본인의 변경 규칙을 똑같이 적용해 보라. 그쪽 형태가 어색하면 원본이 표준이라는 강한 신호 → skip. 특히 '르 / ㅂ / ㄷ / 우' 불규칙 활용은 모델이 자주 헷갈리는 영역이므로 보수적으로 판단.
6. **최종 자기 의심** — 제안 출력 직전 한 번 더 멈춰서 "내 변경이 표준형 → 비표준형으로 거꾸로 갈 가능성" 을 점검. 자신의 결론이 1% 라도 흔들리면 → skip. 규정 번호는 외워도 적용 방향을 거꾸로 잡는 함정을 의식하라.
...
❌ 모르는 어휘를 다른 어휘로 바꾸는 것 (의미 치환은 맞춤법 X)
