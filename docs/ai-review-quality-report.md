...
핵심 KPI:
- 검출률 (10개 함정 중 검출 개수)
- 비용 (크레딧/회, 1크레딧=0.8원, ROUND((원가×1.3)÷0.8))
- 결정론성 (같은 본문 N회 검수 시 결과 일관성)

## 2. 결과 요약

### 2.1 dummy-work-3 (만년취준생) 본문 — 함정 10개
...
### 2.2 dummy-work-2 (메모리언/리운) 본문 — 함정 8개 (작가 추가 검증)

작가가 v3 검수기를 다른 작품으로 검증하기 위해 별도로 8개 함정을 심어 테스트.

| 검수기 | 검출 | 점수 | input tok | output tok | 크레딧 | 비고 |
|---|---|---|---|---|---|---|
| v3 (만년취준생 마무리 시점) | 1/8 | 95 (허위 고점) | 25,449 | 293 | 19 | 새 본문에 v3 자체로 부족 |
| v4 (체크리스트 도입) | 4/8 | 80~83 | 26,234 | 844~883 | 22 | [W번호] 체크리스트로 #2/#3 신규 검출 + 메타 표현(#1) 결정론 |
| v5 (캐릭터 체크리스트) | 5/8 | 70~73 | 27,036 | 965~1,039 | 23 | [C번호] 체크리스트로 #4 (캐릭터 이탈) 신규 검출 |
| v6 (위치 매칭 강화 + 직접 진술) | **7/8 (87.5%)** | 66~68 (결정론) | 27,330 | 1,173~1,289 | 23~24 | #5 위치 + #6 과거 직접 진술 신규 검출 |

미검출 1건: #8 (날짜-요일 불일치) — 결정론 모듈 구현돼 있으나 본문에 명시 짝(`X년 X월 X일 요일`) 없어 적용 대상 없음. 작가가 의도한 "의뢰서 메타데이터(20310422) ↔ 본문 별도 진술 요일" 케이스는 cross-validation 필요(별도 모듈).

탐지율 추이: **12.5% → 50% → 62.5% → 87.5%** (4 라운드 만에 7배 향상).

## 3. 함정 검출 진행

### 3.1 dummy-work-3 (만년취준생)
...
### 3.2 dummy-work-2 (메모리언/리운)

| # | 함정 | v3 | v4 (체크리스트) | v5 (캐릭터 체크리스트) | v6 (최종) | 비고 |
|---|------|----|----|----|----|------|
| 1 | 메타 표현 ("3화에서~") | ❌ | ✅ warning | ✅ | ✅ | 결정론 정규식 모듈 |
| 2 | 보관 상태 확인 30분 (설정: 1시간+) | ❌ | ✅ [W12] | ✅ | ✅ | 세계관 [W번호] 체크리스트 |
| 3 | 자기 쪽 화면 확인 (설정: 불가) | ❌ | ✅ [W8] | ✅ | ✅ | 세계관 [W번호] 체크리스트 |
| 4 | 캐릭터 이탈 ("힘드셨죠") | ❌ | ❌ | ✅ critical [C1] | ✅ | 캐릭터 [C번호] 체크리스트 + 대사 금지 규칙 |
| 5 | 안정제 책상 서랍 (설정: 가방) | ❌ | ❌ | ❌ | ✅ critical [C1] | 단어 단위 매칭 강화 |
| 6 | 과거 직접 진술 ("7년 전~") | ❌ | ❌ | ❌ | ✅ info | 결정론 정규식 모듈 |
| 7 | 성별 지칭 오류 ("그녀" ← 남성) | ✅ | ✅ | ✅ | ✅ | LLM 직접 검출 |
| 8 | 날짜-요일 불일치 | ❌ | ❌ | ❌ | ❌ | 결정론 모듈 구현됐으나 본문에 짝 없음 |
...
#### `ai/app/services/structural_validators.py` (메모리언 검증 후 추가)
LLM attention 분산 한계를 우회하는 결정론 후처리 모듈 묶음:
- `detect_meta_episode_references` — "3화에서~", "N화 때" 같이 작가 시점 메타 회차 참조 검출 (#1)
- `detect_date_weekday_mismatches` — 본문에 명시된 날짜+요일 짝이 캘린더상 모순되는지 검사 (#8)
  - 연도 미명시 시 2020~2035 후보 중 어느 해라도 짝 맞으면 통과
- `detect_past_direct_narration` — "N년 전 …했다/이었다/겪었다" 직접 과거 서술 검출 (#6)
  - style_memo "직접 진술 금지" 작품에서 자주 잡혀야 하는 패턴
- 결과는 review 응답의 `issues` 배열에 머지
...
- 시스템 프롬프트 ~3k → ~8.9k chars (원본 대비 +200%, 메모리언 라운드 포함):
  ...
  - **캐릭터 [C번호] 체크리스트** (메모리언 라운드): 각 인물당 4개 점검 (소지품 위치 / 대사·행동 금지 규칙 / 생활 패턴 / 이름·성별·나이·직업)
  - **세계관 [W번호] 체크리스트** (메모리언 라운드): 각 항목당 4개 점검 (수치 / 금지·제약 / 위치·소지품 / 부수 효과)
  - 위치 매칭에서 "단어 단위 비교 필수, 비슷한 위치로 합리화 금지" 명시 (#5 케이스 직접 겨냥)
...
- (개발 디버깅용) `REVIEW_DEBUG_DUMP=1` 환경변수 시 결과 JSON + 시스템/유저 프롬프트를 `test_reports/last_review.json` / `last_review_prompt.txt`에 덤프 — 프로덕션 미설정 시 0 영향 (테스트 종료 후 제거 예정)
...
가격: `$3/M input, $15/M output` (1 USD ≈ 1450 KRW, 1 credit = 0.8 KRW, ROUND((원가×1.3)÷0.8))
...
### v6 후 (메모리언 검증 마무리)
> "8건 중 7건 탐지. 탐지율 87.5%. 수정 전 12.5%에서 시작해서 여기까지 왔어요.
> 후처리 모듈 분리 판단이 정확했어요 — LLM이 잘 못 하는 걸 LLM한테 계속 시키는 대신
> 규칙 기반으로 뺄 건 빼는 엔지니어링 판단이에요.
> 캐릭터 행동 규칙 체크가 작동하기 시작한 건 큰 진전이에요. 설정집을 잘 채운 작가일수록
> 검수 품질이 올라가는 구조가 만들어지고 있어요."

## 9. 후속 작업 (follow-up 티켓 후보)

### A. 페르소나 톤 검사 (조건부 페르소나 처리) — dummy-work-3 기준
- 캐릭터 메모의 "신안 발동 시", "전투 시" 같은 조건부 페르소나를 본문 장면 상태와 매칭해 검사
- 시도해도 효과 불확실 (false positive 위험), 우선순위 중
- 메모리언 본문에서는 [C1] 체크리스트로 무조건적 페르소나 위반(#4)이 잡힘 — 조건부만 남은 약점

### B. 룰 부재 검사 — dummy-work-3 기준
- worldnotes에 "능력 사용 후 반동" 같은 시스템 룰이 있을 때 본문이 누락하는지 검출
- LLM 약점이라 결정론 보조 모듈 필요. 우선순위 낮음.

### F. 회차별 수치 메타데이터 — dummy-work-3 기준
- 시청자 수, 후원 누적, 시간 경과 등을 회차별로 추적하는 메타데이터 시스템
- 작품마다 필요한 수치가 다르므로 일반화 어려움. 우선순위 낮음.

### 의뢰서 메타-본문 cross-validation — dummy-work-2 #8 기준
- 본문에 직접 명시된 "X년 X월 X일 요일" 짝은 이미 `detect_date_weekday_mismatches`로 잡힘
- 작가가 의도한 케이스: 의뢰서 헤더 메타데이터(예: 20310422)와 본문 별도 진술 요일을 cross-check
- 전용 매처 필요. 작가 코멘트: "실제 작가가 날짜-요일 불일치를 범하는 빈도가 낮을 거라 우선순위 낮음"

### 운영 보호
- review가 정상 종료 시 `REVIEW_DEBUG_DUMP` 블록 제거 (테스트 도구는 main 머지 전 정리)
- 시스템 프롬프트가 8.9k chars로 늘어난 만큼 캐싱(Anthropic prompt caching, cache_control) 검토 — input 토큰 ~10k 감소 예상

## 10. 결론

이번 브랜치는 두 작품 본문에서 검수 품질을 끌어올렸다:
- dummy-work-3 (만년취준생): **5/10 → 7/10**, 점수 결정론 + 1차 검수 안정성 확보
- dummy-work-2 (메모리언/리운, 작가 추가 검증): **1/8 → 7/8 (87.5%)**

비용은 검수당 19~28 크레딧 수준에서 안정. 작가는 두 작품 모두에서 검수기를 "실서비스 신뢰 가능 수준"으로 평가.

핵심 학습:
- LLM 검수기의 약점(빈도 카운트, 회차 누적, 메타 표현, 직접 진술)은 **결정론 후처리 모듈**로 우회하는 게 효율적 — 토큰 비용 0, 정확도 100%
- LLM의 attention 분산 한계는 **번호 라벨 + 체크리스트 강제**로 우회 가능 — `[C번호]`/`[W번호]`로 항목별 1:1 검사 강제하면 detection 큰 폭 향상
- 검수 모드는 컨텍스트 결정론성이 작품 신뢰도와 직결됨 — RAG 트리밍으로 데이터 손실 시 검출 일관성 깨짐 (`PROTECTED_KEYS_BY_MODE`로 해결)
- 프롬프트 강화는 "절차 명시(사전 스캔) + 구체 예시 + 단어 단위 매칭 강조 + 출력 금지"가 효과적
- "설정집을 잘 채울수록 검수 품질이 올라가는" 선순환 구조 확보 — 작가가 설정 입력에 시간 투자할 동기 마련

---
**작업자**: AI 팀 (S14P31F203-132)
**테스트 데이터**:
- dummy-work-3 7화 (5,300자, 함정 10개)
- dummy-work-2 13화 (5,500자, 함정 8개) — 작가 추가 검증
**총 검수 호출**: 약 25회 (전 비용 ~₩3,800, ~600 크레딧)
