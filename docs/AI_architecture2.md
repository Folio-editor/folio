...
Folio 작가가 긴 연재 소설을 쓰는 과정에서, **설정·전개·톤 일관성을 AI가 자동으로 유지**하도록 돕는 서버를 구축한다. 회차 저장 시점부터 청킹·임베딩·요약이 자동 수행되고, 초안 생성과 검수는 RAG로 과거 맥락을 참조해 정확도를 확보한다.
...
| # | 기능 | 트리거 | 모델 | 출력 |
|---|------|--------|------|------|
| ① | **회차 인덱싱 (+ 설정집 자동 추출)** | 자동 저장(5초) → 디바운스(5초) | embedding + Haiku | `episode_chunk`, `episode_summary`, `extraction_suggestion` |
| ② | **초안 생성** | 작가 요청(POST /v1/drafts) | Sonnet 4.5 | SSE 스트리밍 본문(목표 3~5줄 MVP, 확장 시 5,000자) |
| ③ | **검수 (맞춤법 포함)** | 작가 요청(POST /v1/reviews) | Sonnet 4.5 | JSON(issues, newItems) — issue 타입에 `spelling` 포함 |

> **설정집 자동 추출은 ①에 포함**되어 별도 엔드포인트 없음. 요약 LLM 호출 1회에서 함께 산출된 신규 인물/용어/복선을 `extraction_suggestion`(status='pending')으로 저장. 작가 승인 후 `character`/`world_note`/`foreshadow`로 이관.
>
> **맞춤법 검사는 ③에 통합**. 별도 엔드포인트 없이 검수 프롬프트의 RAG 컨텍스트에 고유명사 화이트리스트(캐릭터·세계관 이름)를 포함시켜 검수와 동시에 처리.
>
> **임포트 전용 풀 프로필 자동 생성은 MVP에서 제외**. 임포트 시에도 일반 저장과 동일한 인덱싱 파이프라인을 회차 단위로 N번 돌리며, `extraction_suggestion`이 자연스럽게 누적됨. 풀 프로필이 필요하면 작가가 수기로 보강.
...
### 4.4 설정집 자동 추출 (①에 통합된 부가 기능)

별도 기능이 아니라 **회차 인덱싱 파이프라인의 마지막 단계(`extract_items`)** 에서 나오는 산출물. Haiku 요약 결과의 `new_terms` / `characters.new` / `foreshadowing`을 신규 항목으로 `extraction_suggestion`에 적재한다.

```
[Celery chain의 extract_items.s()]
   - 직전 단계(generate_summary)의 JSON에서 신규 항목 추출:
     · characters.new           → entity_type='character'
     · new_terms                → entity_type='term' (world_note로 이관)
     · foreshadowing            → entity_type='foreshadow'
   - 기존 설정집과 중복 체크 (이름 매칭 + character_embedding 유사도)
   - extraction_suggestion INSERT (status='pending', is_auto_generated=true,
                                    source_episodes=[episode_id])
       ↓
[작가 승인 UI: /works/{id}/suggestions]
   - 목록 표시 + [승인][수정 후 승인][거부]
   - 승인 → Spring이 character/world_note/foreshadow 테이블로 이관
            (is_auto_generated=true 플래그 유지)
```

**특징**:
- 추가 LLM 호출 없음 — 요약 1회 호출에서 같이 뽑음 → **비용 0**
- 얕은 추출 (이름 + 한 줄 설명 수준). 풀 프로필이 필요하면 작가가 승인 후 직접 보강.
- 임포트 시에도 동일한 흐름을 회차 단위로 N번 반복 — 임포트 전용 로직 없음.
