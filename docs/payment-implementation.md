...
Folio의 AI 토큰 경제(종량제 + 프로 구독)를 토스페이먼츠 기반으로 구현한다.
...
Folio 기획안 v4에서 확정된 토큰 경제를 그대로 반영한다.
...
Folio는 무료 체험이나 예약 구독이 기획에 없으므로 **`created_at`만으로 충분하다**. `started_at`은 향후 체험 기능 도입 시 추가.
