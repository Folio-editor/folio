...
CREATE TABLE archive_tag (
    id              UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    archive_id      UUID NOT NULL REFERENCES idea_archive(id) ON DELETE CASCADE,
...
CREATE TABLE dictionary (
    ...
    work_id         UUID NOT NULL REFERENCES work(id) ON DELETE CASCADE,
    writer_id       UUID NOT NULL REFERENCES writer(id) ON DELETE CASCADE,
    world_note_id   UUID REFERENCES world_note(id) ON DELETE SET NULL,
...
CREATE INDEX idx_episode_version_episode ON episode_version(episode_id);
...
CREATE INDEX idx_archive_tag_archive ON archive_tag(archive_id);
CREATE INDEX idx_dictionary_work ON dictionary(work_id);
