...
| status | VARCHAR(20) | NOT NULL | 연재중/완결/휴재 등 |
...
| is_template | BOOLEAN | NOT NULL DEFAULT false | 기본 템플릿 여부 |
| sort_order | INTEGER | NOT NULL | 형제 노트 간 정렬 |
| created_at | TIMESTAMP | NOT NULL | |
| updated_at | TIMESTAMP | NOT NULL | |

**기본 템플릿 (워크스페이스 생성 시 자동 생성):**
...
| episode_number | INTEGER | | 회차 번호 (막 노드는 NULL) |
...
| status | VARCHAR(20) | NOT NULL | 미작성/초고/퇴고/완성 |
...
| content_diff | TEXT | | 변경 내용 또는 전체 스냅샷 |
| word_count | INTEGER | NOT NULL | |
| saved_at | TIMESTAMP | NOT NULL | |

---

### 1.10 PlotEpisodeLink (플롯 ↔ 원고 링크)
...
| work_id | UUID | FK → Work | |
| writer_id | UUID | FK → Writer | |
| title | VARCHAR(200) | NOT NULL | 핵심 내용 한 줄 |
| content | TEXT | | TipTap JSON 보충 설명 |
| sort_order | INTEGER | NOT NULL | |
| created_at | TIMESTAMP | NOT NULL | |
| updated_at | TIMESTAMP | NOT NULL | |
...
|------|------|------|------|
| id | UUID | PK | |
| archive_id | UUID | FK → IdeaArchive | |
| tag | VARCHAR(100) | NOT NULL | 문장/장면/설정/반전/대사 + 사용자 추가 |
| created_at | TIMESTAMP | NOT NULL | |

---

### 1.15 Dictionary (용어 사전)
...
| work_id | UUID | FK → Work | |
| writer_id | UUID | FK → Writer | |
| world_note_id | UUID | FK → WorldNote, NULL | 세계관 노트 참조 시 |
| character_id | UUID | FK → Character, NULL | 등장인물 참조 시 |
| word | VARCHAR(200) | NOT NULL | 용어 |
| type | VARCHAR(20) | NOT NULL | 용어 유형 |
| created_at | TIMESTAMP | NOT NULL | |
| updated_at | TIMESTAMP | NOT NULL | |

---

### 1.16 CharacterRelation (인물 관계 — 추후 구현)

인물 간 관계 정의. 관계도 기능 구현 시 활성화.
...
| work_id | UUID | FK → Work | |
| writer_id | UUID | FK → Writer | |
| from_character_id | UUID | FK → Character | |
| to_character_id | UUID | FK → Character | |
| relation_type | VARCHAR(100) | NOT NULL | 동맹/적대/연인/가족 등 |
...
### 소프트 삭제
- Writer만 `deleted_at` 소프트 삭제 적용
- 나머지 엔티티는 하드 삭제 (동기화 시 PowerSync가 삭제 전파)
