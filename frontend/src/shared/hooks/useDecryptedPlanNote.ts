...
export interface DecryptedPlanNoteRow {
  id: string;
  work_id: string;
  writer_id: string;
  title: string;
...
export function useDecryptedPlanNoteList(rawRows: RawPlanNoteRow[]): {
  data: DecryptedPlanNoteRow[];
  isLoading: boolean;
} {
  const [decrypted, setDecrypted] = useState<DecryptedPlanNoteRow[] | null>(null);
