 ...
 *
 * 현재(2026-05) 운영자 페이지는 localStorage 런타임 토큰 입력 방식으로 전환됨.
 * 즉 빌드 시점 환경변수 주입은 더 이상 사용하지 않는다.
 *
 * 이 스크립트는 다음 상황 방어용으로 유지:
 * - 누군가 실수로 VITE_ADMIN_API_TOKEN 을 코드에 다시 import 했을 때
 * - 빌드 환경변수에 VITE_ADMIN_API_TOKEN 이 주입된 상태로 빌드 시
 * 산출물에 평문이 박히면 즉시 빌드 실패시켜 사고 방지.
 *
